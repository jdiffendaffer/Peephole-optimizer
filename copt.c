...
/* Altered file IO to work as a standalone optimizer for use with AGD.  J. Diffendaffer 2022 */
...
*/
    ...
    if ((inp = fopen(argv[1], "r")) == NULL)
        error("copt: can't open assembly file\n");
	// open the output file
    if ((outf = fopen(argv[2], "w")) == NULL)
        error("copt: can't open output file\n");
	// open the optimizer patterns file
...
//    printlines(head.l_next, &tail, stdout);
    printlines(head.l_next, &tail, outf);
